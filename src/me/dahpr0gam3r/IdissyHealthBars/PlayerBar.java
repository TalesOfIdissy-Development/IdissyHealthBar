...
import me.dahpr0gam3r.IdissyHealthBars.api.BarHideEvent;
...
public class PlayerBar {
	...
	public static boolean hasHealthDisplayed(Player player) {
		...
		if (sb.getPlayerTeam((OfflinePlayer)player).getName().contains("ibr")) return true;
		return false;
	}
	
	public static void hideHealthBar (Player player) {
		Team team = sb.getTeam("ibr0");
		if (team == null) {
			team = sb.registerNewTeam("ibr0");
	...
	public static void setHealthSuffix (Player player, double health, double max) {
		...
		if (useCustomBar || (!textMode)) {
			...
			sb.getTeam("ibr" + Integer.toString(healthOn10)).addPlayer((op));
			return;
		} else {
			...
			Team team = sb.getTeam("ibr" + intHealth + "-" + intMax);
			if (team == null) {
				team = sb.registerNewTeam("ibr" + intHealth + "-" + intMax);
	...
	public static String getColor (double health, double max) {
		...
		if (ratio > 0.5) 	return ChatColor.translateAlternateColorCodes('&',"&a"); //more than half health -> green
		if (ratio > 0.25) 	return ChatColor.translateAlternateColorCodes('&',"&e"); //more than quarter health -> yellow
		return ChatColor.translateAlternateColorCodes('&',"&c"); //critical health -> red
	}
