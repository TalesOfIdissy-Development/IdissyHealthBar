...
import me.dahpr0gam3r.IdissyHealthBars.api.HealthBarAPI;
		...
		if(damageEvent instanceof EntityDamageByEntityEvent) {
			...
			if (deathMessage.contains("shot") || deathMessage.contains("shooted")) {
				if (damager instanceof Projectile) {
					...
					if (shooter instanceof LivingEntity) {
						event.setDeathMessage(victim + " was shot by " + ((LivingEntity) shooter).getName());
						return;
					}
				}
			}
			...
			if (deathMessage.contains("fireballed")) {
				if (damager instanceof Projectile) {
					...
					if (shooter instanceof LivingEntity) {
						event.setDeathMessage(victim + "was fireballed by " + ((LivingEntity) shooter).getName());
						return;
					}
				}
			}
